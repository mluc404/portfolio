...
export default function JobCard({ job }: { job: JobProps }) {
  return (
    ...
    >
      {" "}
      <div className="text-gray-400 text-[1rem] md:text-[1rem]">{job.time}</div>
      <div className="flex flex-col">
        <div className="font-semibold text-[1.1rem] md:text-[1.1rem]">
          {job.title}
        </div>
        <div className="md:text-[1rem]">{job.company}</div>
        <div className="text-gray-400 leading-normal my-2 mb-3 md:text-[1rem]">
          {job.description}
        </div>
        <div className="flex gap-2 flex-wrap">
          {job.skills.map((skill, index) => (
            ...
            text-[.9rem] md:text-[.8rem] text-cyan-300"
            >
              {skill}
            </div>
          ))}
